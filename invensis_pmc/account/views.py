...
from inhouseapp.settings import APPLICATION_URL, APPLICATION_FROM_EMAIL
from django.utils import timezone
from inhouseapp.utils.decorators import log, anonymous_required
from . import models
from . import forms


def index(request, template="account/index.html", context={}):
    return render(request, template, context)


@log
@anonymous_required
def login(request, template="account/login.html", context={}):
            ...
            if user is not None:
                if user.is_active:
                    ...
                    if request.user.groups.filter(name='sales manager').exists():
                        return redirect(reverse('sales_manager_dashboard'))
                    
                    elif request.user.groups.filter(name='sales rep').exists():
                        return redirect(reverse('sales_rep_dashboard'))
                    
                    elif request.user.groups.filter(name='operations manager').exists():
                        return redirect(reverse('operation_manager_dashboard'))

                    elif request.user.groups.filter(name='lead').exists():
                        return redirect(reverse('lead_dashboard'))
                        ...
                        return redirect(reverse('qc_dashboard'))
...
@log
@anonymous_required
def register(request, template="account/register.html", context={}):
    ...
    if request.method == 'POST':
        if user_form.is_valid() and profile_form.is_valid():
            ...
            messages.add_message(request, messages.ERROR, _('Please fill the required fields bellow.'))
...
def request_confirm(request, activation_key, template='account/password_reset.html', context={}):
    ...
    if user_request.is_approved or user_request.key_expires_at < timezone.now():
        return render_to_response('account/token_invalid.html')
...
@log
@login_required
def change_password(request, template="account/password_change.html", context={}):
    ...
    if request.method == 'POST':
        ...
        if change_password_form.is_valid():
            ...
            messages.add_message(request, messages.ERROR, _('Some errors occurred.'
                                                            '\nPlease fill the required fields bellow.'))
    else:
...
@log
def modify_account(request, template="account/account_modify.html", context={}):
    user_form = None
    profile_form = None
...
@log
def restore_password(request, template="account/password_restore.html", context={}):
...
@log
@login_required
def change_email(request, template="account/email_change.html", context={}):
